#' Visualization heatmap of cluster data of sce
#'
...
#' @examples
#'
#' sce.file <- system.file("extdata/sce.rds", package = "hyperion")
#' sce <- readRDS(file = sce.file)
#'
#' plotClusterHeatmap(sce)
#' plotClusterHeatmap(sce, color = colorRampPalette(c("purple","white","yellow"))(100))
#' plotClusterHeatmap(sce, cluster_row = FALSE)
#' plotClusterHeatmap(sce, cluster_row = FALSE, cluster_col = FALSE)
#'
#'
#'
plotClusterHeatmap <- function(object,
  ...
  mat <- plot_meta.data[, object@markers]
  rownames(mat) <- plot_meta.data$cluster
...
#'
#' Visualization heatmap of branch data of sce
#'
...
#' plotBranchHeatmap(sce, color = colorRampPalette(c("purple","white","yellow"))(100))
#' plotBranchHeatmap(sce, cluster_row = FALSE)
#' plotBranchHeatmap(sce, cluster_row = FALSE, cluster_col = FALSE)
#'
#'
#'
plotBranchHeatmap <- function(object,
  ...
  plot_meta.data <- fetchClustMeta(object, verbose = FALSE)
  branch = NULL
  mat <- aggregate(plot_meta.data[, object@markers], list(branch = plot_meta.data[, "branch.id"]), mean)
...
#'
#' Visualization heatmap of intermediate cells of sce
#'
...
plotTrajHeatmap <- function(object,
  ...
  # update plot meta information
  plot_meta.data <- fetchClustMeta(object, verbose = FALSE)
  if (sum(plot_meta.data$traj.value.log) == 0) {
    stop(Sys.time(), " please run runWalk first")
  }
  plot_meta.data <- plot_meta.data[plot_meta.data$traj.value.log > cutoff, ]
  plot_meta.data <- plot_meta.data[order(plot_meta.data$pseudotime), ]
  ...
  mat <- plot_meta[match(plot_meta.data$cell, rownames(plot_meta)), object@markers.idx]
  gg <- pheatmap(t(mat), color = color, scale = scale, border_color = NA, ...)
...
#'
#' plotHeatmap(sce)
#' plotHeatmap(sce, cluster_rows = TRUE)
#' plotHeatmap(sce, cluster_rows = TRUE, clustering_method = "ward.D")
#' plotHeatmap(sce, cluster_rows = TRUE, cluster_cols = TRUE)
#'
#'
...
plotHeatmap <- function(object,
  ...
  # update plot meta information
  plot_meta.data <- fetchPlotMeta(object, verbose = FALSE)

  if (downsize > dim(plot_meta.data)[1]) {
    downsize = dim(plot_meta.data)[1]
  }
  plot_meta.data <- plot_meta.data[sample(seq_len(dim(plot_meta.data)[1]), downsize), ]

  if (max(plot_meta.data$pseudotime) > 0) plot_meta.data <- plot_meta.data[order(plot_meta.data$pseudotime), ]

  mat <- plot_meta[match(plot_meta.data$cell, rownames(plot_meta)), ]
  if (is.null(markers)) {
    markers <- object@markers
  } else {
    markers <- markers[markers %in% object@markers]
...
heatmap_anno_combined <- function(sce, keycol = "cell_type10",
  ...
  c_mat_lst <- hyperion::fetch_heatmap_dat(sce, cluster.id = keycol)
  c_mat <- c_mat_lst[[1]]
