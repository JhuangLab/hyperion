...
make_rds <- function(stype = "normal",
                     ...
                     project = "hyperion",
                     overwrite = F,
                     filter_markers = c("80ArAr", "120Sn", "127I", "134Xe", "138Ba", "DNA1", "DNA2", "Histone3", "208Pb"),
                                       glob = NULL){
  if(is.null(glob)){
  ...
  analysis_dir <- glue("~/projects/{project}/analysis/{dataset}/human/steinbock")
  ...
  if(fs::file_exists(rds_fn) && !overwrite){
    ...
    if(is.null(glob)){
      para_samples <- dir_ls(glue("~/projects/{project}/analysis/{dataset}/human/steinbock/{stype}/measure")) %>% path_file()
    }else{
      para_samples <- dir_ls(glue("~/projects/{project}/analysis/{dataset}/human/steinbock/{stype}/measure"), glob = glob) %>% path_file()
    }
    ...
    sce_lst <- punc_sce %>% map( ~ filter_markers(.x,filter_markers))
